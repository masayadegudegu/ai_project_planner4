...
const App: React.FC = () => {
  ...
  // Auth and project modals
  const [isAuthModalOpen, setIsAuthModalOpen] = useState<boolean>(false);
  const [authMode, setAuthMode] = useState<'signin' | 'signup'>('signin');
  const [isProjectLibraryOpen, setIsProjectLibraryOpen] = useState<boolean>(false);
  const [isProjectSaveModalOpen, setIsProjectSaveModalOpen] = useState<boolean>(false);
  ...
  // Show auth modal if user is not logged in and not loading
  useEffect(() => {
    if (!authLoading && !user && currentView !== ViewState.INPUT_FORM) {
  ...
  const handleSaveProject = () => {
    ...
    setIsProjectSaveModalOpen(true);
  };
  ...
  const handleLoadProject = (goal: string, date: string, projectTasks: ProjectTask[], projectGanttData: GanttItem[] | null, projectId?: string) => {
    setProjectGoal(goal);
    setTargetDate(date);
