...
const ProjectFlowDisplay: React.FC<ProjectFlowDisplayProps> = ({ 
  ...
  onUpdateTaskStatus, onStartNewProject, onExportProject, onSaveProject, onOpenProjectLibrary, onAddTask, onRemoveTask, onImportSingleTask, 
  onAutoLayout, onUndo, canUndo, onRedo, canRedo,
  generateUniqueId, onUpdateTaskConnections,
  ganttData, setGanttData, onCustomReportGenerated, onClearApiKey, currentProjectId, user
}) => {
      ...
      <div className="max-w-full mx-auto w-full">
        <div className="flex flex-wrap items-center justify-between gap-4 mb-6">
          ...
          <div className="flex flex-wrap gap-2 items-center">
            {user && <UserMenu onOpenProjectLibrary={onOpenProjectLibrary} onClearApiKey={onClearApiKey} />}
            <button
            ...
            ><DownloadIcon className="w-5 h-5 mr-2" />{currentProjectId ? '保存' : '保存・エクスポート'}</button>
            <button
           ...
           <div className="flex flex-wrap justify-between items-start gap-4">
              <div>
                ...
                <div className="space-y-2 text-slate-700">
                  ...
                  {currentProjectId && <p className="text-sm text-slate-500">プロジェクトID: {currentProjectId}</p>}
                </div>
              </div>
