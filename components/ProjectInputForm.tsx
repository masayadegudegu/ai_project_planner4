...
import { TargetIcon, CalendarIcon, UploadIcon, FolderIcon, SparklesIcon } from './icons';
    ...
    <div className="min-h-screen flex flex-col items-center justify-center bg-gradient-to-br from-slate-900 to-slate-700 p-4">
      ...
      {user && (
        <div className="absolute top-4 right-4">
          <UserMenu onOpenProjectLibrary={onOpenProjectLibrary} onClearApiKey={() => {}} />
        </div>
      )}
      ...
      <div className="w-full max-w-2xl bg-white shadow-2xl rounded-xl p-8 md:p-12">
        ...
        <form onSubmit={handleSubmit} className="space-y-6">
          ...
          <div className="flex justify-center">
            <button
              type="button"
              onClick={user ? onOpenProjectLibrary : onOpenAuth}
              className="flex items-center gap-2 px-6 py-3 text-sm font-medium text-slate-700 bg-slate-100 border border-slate-300 rounded-lg hover:bg-slate-200 transition-colors"
            >
              <FolderIcon className="w-5 h-5" />
              {user ? 'プロジェクトライブラリを開く' : 'ログインしてプロジェクトを管理'}
            </button>
          </div>
